  ...
  async function (req, res) {
    ...
    //  主持下的店铺明细
      ...
      for (const dpName in dianpuGroup) {
        ...
        let zrce = new Decimal(0)
        let index = dpzjtjList.length + 2
        dpzjtjList.push({rq: date, dpm: dpName, jrxj: totalPrice, zrce: zrce, zj: sum.plus(zrce).toFixed(2),
            gs: `=A${index}&"  今日合计"&C${index}&"， 昨日差额"&D${index}&"，  合计差额"&E${index}`})
      }
